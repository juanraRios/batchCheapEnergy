...
public class RESTIndicatorJobLauncher {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(RESTIndicatorJobLauncher.class);
    ...
    @Autowired
    RESTIndicatorJobLauncher(@Qualifier("restIndicatorJob") Job job, JobLauncher jobLauncher) {
        this.job = job;
    ...
    void launchXmlFileToDatabaseJob() throws JobParametersInvalidException, JobExecutionAlreadyRunningException, JobRestartException, JobInstanceAlreadyCompleteException {
        LOGGER.info("Starting restIndicatorJob job");

        jobLauncher.run(job, newExecution());

        LOGGER.info("Stopping restIndicatorJob job");
    }
